...
#
#            a) for k in range(r - 1):
#            b) for k in range(r):
#            c) for k in range(r + 1):
#
#               answer: b
#
...
#                  count = 0                should be before loop
#                  if math.cos(r) > 0:      should be r + k
#                      count = 1            should be count = count + 1
#                  return count             should be after loop
#
#     f. The code in the "graphics accumulation" example below includes:
#            for _ in range(n):
#        What does the   _   (underscore) mean? A loop variable not used in a loop
#
...
#        and one is WRONG.  Which is the WRONG one? The bottom one os wrong because it tries to use k and the x and y's
#
#              x = starting_point.x
#              for k in range(n):
